...
function addRefreshButton() {
    ...
    const navigation = document.getElementById('navigation');
    if (navigation) {
        navigation.appendChild(refreshBtn);
    }
}
...
// Populate All Tasks - Fixed version
function populateAllTasks() {
  allTasksContainer.innerHTML = '';
  
  // Show all tasks that staff can see (assigned to them + shared with them + available for takeover)
  const visibleTasks = tasks.filter(task => 
    task.assignedTo === currentUser.username || 
    task.sharedWith.includes(currentUser.username) ||
    true // Show all tasks for takeover consideration
  );
...
// Populate My Tasks - Fixed version
...
// Populate Extra Tasks - Fixed version
...
// Create Task Card for Staff Panel - Fixed version
function createTaskCard(task, type) {
  ...
  if (canEdit) {
    ...
    taskCard.querySelector('.cancel-btn').addEventListener('click', () => cancelTask(task.id));
...
function takeOverTask(taskId) {
  ...
  .then(response => {
    if (response.success) {
      ...
      // Refresh tasks from backend to get updated data
  ...
  .then(() => {
    // Update staff panel if we're on that page
    if (currentPage === 'staff-panel') {
      updateStaffPanel();
    }
  })
